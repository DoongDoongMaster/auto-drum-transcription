...
def upload_file():
    ...
    result = segment_classify.predict(file_path, bpm, delay)
    ...
    if len(result["rhythm"]):
        ...
        result["rhythm"][current_bar_idx] = list(
            map(float, result["rhythm"][current_bar_idx])
        )
    ...
    return json.dumps({"instrument": instrument, "rhythm": result["rhythm"]})
